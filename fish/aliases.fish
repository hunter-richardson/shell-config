...
alias dtrx '/home/RichardsonHR/git-repos/dtrx/scripts/dtrx -forv --one-entry=inside'
alias egrep 'command egrep --color=auto'
alias exit 'nohup rm ~/tmux-{client,server}-*.log &; command tmux kill-server'
alias fgrep 'command fgrep --color=auto'
alias gclone 'cd ~/git-repos; for i in $argv; command git clone --verbose --depth 1 -c http.sslverify=false $i (builtin printf "./%s" (builtin string split -r -m1 "/" $i | command cut -d" " -f1)[-1]); end; cd -'
