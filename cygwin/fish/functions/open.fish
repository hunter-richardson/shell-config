...
function open
  if builtin test -n $argv[1]
    builtin set -l err " ... the door is either locked or gone!"
  else
    builtin echo "Open what?"
    builtin return 1
  end
  ...
  switch $type;
      case b
        ...
        if builtin test -n $mnt -a -r $mnt
          eval $_ $argv[1]
        else
          builtin printf '%s%s%s%s:  %s' $bold $red $argv[1] $normal $err
          builtin return 1
        end
      case c s
        command cat $argv[1]
      ...
      case l
        ...
        if builtin test -n $lnk -a -r $lnk;
          eval $_ $lnk
        else
          builtin printf '%s%s%s%s%s:  %s' $red $bold $argv[1] $normal $err
          builtin return 1
        end
      case p
        command cat < $argv[1]
