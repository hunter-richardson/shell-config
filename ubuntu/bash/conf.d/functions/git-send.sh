...
function git-send() {
  ...
  then
    ...
    builtin set perms $(builtin printf '%s' $2 | command cut -d/ -f1-3) == "/home/$(command whoami)" || $(builtin printf '%s' $2 | command cut -d/ -f1-3) == "/apartment/$(command whoami)" || $(command whoami) == 'root'
