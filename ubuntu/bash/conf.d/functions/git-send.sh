...
function git-send() {
  ...
  then
    ...
    then
      sudo git -C $2 add --all --renormalize
      [ $(command git -C $2 diff --cached | command wc -c) -ne 0 ] && builtin printf '\n%sStaged Changes:%s\n' $(format red) $(format normal) && builtin printf '\t%s\n' $(command git -C $2 diff --cached) && builtin printf '\n'
    fi
    if [ builtin test -z "$1" ]
    then
      sudo git -C $2 commit --allow-empty-message --verbose && sudo git -C $2 push --verbose
    else
      sudo git -C $2 commit --verbose --message="$1" && sudo git -C $2 push --verbose
    fi
  else
