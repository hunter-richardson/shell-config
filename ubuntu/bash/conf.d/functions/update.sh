...
function update() {
  ...
  function __update_git() {
    ...
    do
      ...
      [ $i == 'hunter-richardson/shell-config/.git' ] && builtin source /etc/bash/config.sh && command tmux source /etc/tmux/conf || [ $i == 'tmux-plugins/tpm/.git' ] && command tource /etc/tmux/conf
    done
  }
