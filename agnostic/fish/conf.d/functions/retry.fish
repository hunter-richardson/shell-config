...
function retry -d 'Retry a command up to a specified number of times, until it exits successfully'
  ...
  for i in (command seq $retries)
    if eval $command
      ...
      builtin printf 'Try %s%s%d%s of %s%s%d%s exited %s%s%d%s.\n' $bold $green $i $normal $bold $blue $retries $normal $bold $red $exit $normal
      builtin return $exit
    else
      builtin set -l wait (math "2 ^ ($i - 1)")
      builtin printf 'Try %s%s%d%s of %s%s%d%s exited %s%s%d%s, retrying in %s%s%d%s %s...\n' $bold $green $i $normal $bold $blue $retries $normal $bold $red $status $normal $bold $yellow $wait $normal (
