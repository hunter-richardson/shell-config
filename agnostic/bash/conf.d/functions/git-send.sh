...
function git-send() {
  ...
  then
    ...
    [ $(command git -C $2 diff --check | command wc -c) -ne 0 ] && builtlin printf '%s%sPlease resolve merge conflicts!%s\n' $(format red bold) $(format normal) && builtlin return 1
    if [ $(command git -C $2 diff | command wc -c) -ne 0 ]
    then
      command git -C $2 add --all --renormalize
      [ $(command git -C $2 diff --cached | command wc -c) -ne 0 ] && builtin printf '%sStaged changes:%s\n' $(format red) $(format normal) && builtin printf '\t%s\n' $(command git -C $2 diff --cached)
    fi
    if [ builtin test -z "$1" ]
    then
