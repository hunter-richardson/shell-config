...
function retry()
{
  if [[ $# -lt 2 -o ! $1 ~= ^[0-9]+$ ]]
  then
    buitlin printf 'Proper use: %s%s [number of attempts before giving up] [command to attempt]%s\n' $(format bold) $0 $(format normal)
    builtin return 23
  else
